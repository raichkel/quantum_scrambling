...
function main(T=5.0, N=21; H="MF", Δ=1 )
    ...
    plot(times, chi, label=false, framestyle=:box)  
    ...
    plot(times,log.(C_r_t_5), label="r=5", left_margin=20px, framestyle=:box) #, ylim=(min_y, 20)
    ...
    plot!(times,log.(C_r_t_60), label="r=60", left_margin=20px)

    plot!(times,log.(C_r_t_70), label="r=70", left_margin=20px)

    plot!(times,log.(C_r_t_80), label="r=80", left_margin=20px)

    plot!(times,log.(C_r_t_90), label="r=90", left_margin=20px)
    ...
    t_min_plot= Int(tmin/δt) # index where t = 5.0 = tmin
    

    # need to change this so C_r_t_00 are same length as c_arr because they use different timescales 
    plot_40 = C_r_t_40[t_min_plot+1:length(C_r_t_40)]
    plot_50 = C_r_t_50[t_min_plot+1:length(C_r_t_50)]
    plot_60 = C_r_t_60[t_min_plot+1:length(C_r_t_60)]
    plot_70 = C_r_t_70[t_min_plot+1:length(C_r_t_70)]
    plot_80 = C_r_t_80[t_min_plot+1:length(C_r_t_80)]
    plot_90 = C_r_t_90[t_min_plot+1:length(C_r_t_90)]
    

   

    plot(log.(c_arr[1,:]),log.(plot_40), left_margin=20px,framestyle=:box)
    plot!(log.(c_arr[2,:]),log.(plot_50), left_margin=20px)
    plot!(log.(c_arr[3,:]),log.(plot_60), left_margin=20px)
    plot!(log.(c_arr[4,:]),log.(plot_70), left_margin=20px)
    plot!(log.(c_arr[5,:]),log.(plot_80), left_margin=20px)
    plot!(log.(c_arr[4,:]),log.(plot_90), left_margin=20px)
